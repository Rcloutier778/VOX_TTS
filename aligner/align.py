...
def viterbi(input_mlf, word_dictionary, output_mlf, phoneset, hmmdir):
    ...
    cmd = ['HVite','-T','1','-a','-m',
        ...
        '-S','./tmp/test.scp',
        '-i',output_mlf,
        '-p','0.0','-s','5.0',word_dictionary,phoneset
        ]
    f = open('./tmp/aligned.results','w+')
    proc = subprocess.Popen(cmd, stdout=f, stderr=subprocess.PIPE)
    out, err = proc.communicate()
    if err:
        print('Got error in viterbi!')
        print(err)
...
def run(wavfile=None, outfile=None, trsfile=None):
    global sr_models
    ...
    if not wavfile:
        ...
        wavfile, outfile = args[:2]
    if trsfile:
        trsfile = wavfile.rsplit('/',1)[1][:-4]
    else:
    ...
    alignedMLF = readAlignedMLF(output_mlf, SR, float(wave_start))
    writeTextGrid(outfile, alignedMLF)

def run_many_child(q):
    while True:
        if not q.empty():
            path = q.get()
            ...
            return
        try:
            run(path, os.path.join("%s_out" % dir, "%s.%s" % (name, 'TextGrid',)), True)
        except Exception as e:
            print('Got exception for %s as follows:\n%s'%(name, e))

def run_many(voice='VOX'):
    "Parallel runs"
    assert voice in ['HECU','VOX']
    import shutil
    
    shutil.rmtree(r'C:\Users\Richard\Documents\Coding Projects\VOX_TTS\aligner\tmp')
    
    DIR = os.path.join(os.getcwd(), voice)
    
    q = mp.Queue()
    procs = []
    for _ in range(mp.cpu_count()):
        p = mp.Process(target=run_many_child, args=(q,))
        p.start()
    
    for fname in sorted([x for x in os.listdir(DIR) if not x.startswith('_')]):
        q.put(fname)
